...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createStock() {
    ...
    const body = { ...req.body };
    if (body?.portfolio_stock?.length > 0) {
      const create_portfolio_stock = body.portfolio_stock;
      body.portfolio_stock = {
        create: create_portfolio_stock,
      };
    } else {
      delete body.portfolio_stock;
    }
